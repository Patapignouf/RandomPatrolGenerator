...
diag_log format ["Begin generation AO with Hostile civilian %1",_thisAvailablePosition];
...
_baseRadius = 60;
for [{_i = 0}, {_i < round ((_thisDifficulty-1)/2)+1}, {_i = _i + 1}] do 
{
...
if (random 100 < 75) then 
{
	...
	{	
		...
		if (random 100 < 30 && enableArmoredVehicle) then 
		{
			//80% chance to spawn light armored vehicle
...
if (count _thisAvailableCivGroup > 0) then 
{
