...
_RcsTitleDialog ctrlSetText (format ["Rank manager for %1", name _target]);
...
_RcsBodyLeftDialog ctrlSetStructuredText  (parseText ([_target] call getDisplayableCurrentRank));
...
_RcsBodyRightDialog ctrlSetStructuredText  (parseText ([_target] call getDisplayableCurrentXPPerCat));
...
if ([_target] call getPlayerRank == "Colonel") then 
{
	...
	_ButtonLeft ctrlAddEventHandler ["ButtonClick",{
		...
		[[_target], 'GUI\rankGUI\rankGUI.sqf'] remoteExec ['BIS_fnc_execVM', player];
		}];	
